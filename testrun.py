...
# 1. Config
# ==========================================
TARGET_DIR = r"C:\Users\End User\Downloads"
...
def clean_attribute(val, default_val):
    """Cleans up attribute values (e.g., handling lists, NaNs, Nones)."""
    if isinstance(val, list): return val[0] 
    if pd.isna(val) or val is None: return default_val
    return val

def categorize_road(osm_type):
    """Categorizes OSM highway tags into simplified road types."""
    t = str(osm_type)
...
def generate_fake_timestamp(scenario):
    """Generates a realistic timestamp based on the traffic scenario."""
...
def apply_traffic_conditions(G, time_label):
    """
    Applies traffic congestion to the graph and calculates realistic speeds/weights.
    MODIFIED: Now uses a dynamic speed reduction for 'Congested' roads.
    """
    ...
    for u, v, data in G.edges(data=True):
        ...
        # Determine the maximum speed limit (set in main)
        max_speed = data.get('max_speed_kph', 40.0)

        if (u, v) in obstacle_keys:
            # 🚦 Congested Road: Realistic Speed Reduction
            data['traffic_status'] = "Congested"
            data['is_obstacle'] = 1
            
            # Calculate a realistic speed during congestion (e.g., 5-15 kph)
            congested_speed = random.uniform(5, 15) 
            data['current_speed_kph'] = congested_speed 
            
            # Calculate new, higher travel time (weight = length / speed)
            # travel time in seconds = (length in km) / (speed in km/s)
            travel_time_sec = (data['length'] / 1000) / (data['current_speed_kph'] / 3600)
            data['weight'] = travel_time_sec 
            
        else:
            # ✅ Clear Road: Maintain speed limit or slightly below
            data['traffic_status'] = "Clear"
            data['is_obstacle'] = 0
            
            # Calculate clear speed (e.g., 80-100% of max speed)
            clear_speed = max_speed * random.uniform(0.8, 1.0)
            data['current_speed_kph'] = clear_speed
            
            # Recalculate the travel time using the clear speed
            travel_time_sec = (data['length'] / 1000) / (data['current_speed_kph'] / 3600)
            data['weight'] = travel_time_sec
            ...
            try:
                ...
                def extract_row(u, v, d, label):
                    ...
                    return {
                        ...
                        "is_chosen": label,
                        "Speed": float(f"{d['current_speed_kph']:.1f}"),
                        "Road_type": categorize_road(raw_type),
...
def main():
    ...
    # Filter to exclude major high-speed roads
    ...
    G = ox.add_edge_speeds(G) # Use OSMnx's default speed configuration
    
    # -----------------------------------------------------------
    # 🎯 FIX: Move speed/length cleaning BEFORE add_edge_travel_times
    # -----------------------------------------------------------
    # Ensure all edges have a length attribute (required for travel_times)
    if 'length' not in G.edges[list(G.edges)[0]]:
        G = ox.add_edge_lengths(G)

    # Clean up speed data and add a max_speed column for reference BEFORE calculating travel times
    for u, v, d in G.edges(data=True):
        # Ensure 'speed_kph' is a single float
        if isinstance(d.get('speed_kph'), list):
            d['speed_kph'] = d['speed_kph'][0]
        # Fallback speed for unclassified/missing speed roads
        if pd.isna(d['speed_kph']) or d['speed_kph'] is None:
            d['speed_kph'] = 40.0
        # Preserve the maximum speed limit for later comparison
        d['max_speed_kph'] = d['speed_kph'] 

    # This call now works because speed_kph and length are guaranteed to be non-null.
